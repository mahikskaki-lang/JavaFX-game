...
import java.awt.Rectangle; // KEPT: Using AWT Rectangle for collision logic is fine
...
public class Entity {
    ...
    public Rectangle solidArea = new Rectangle(0,0, 48, 48); // KEPT
    public Rectangle attackArea = new Rectangle(0,0, 0, 0); // KEPT
    ...
    protected String rollBasicDropName() {
        int i = new Random().nextInt(100) + 1;
        if (i < 50) return OBJ_Coin_Bronze.objName;
        if (i < 75) return OBJ_Heart.objName;
        return OBJ_ManaCrystal.objName;
    }

    public void checkDrop() {
        // Decide the loot to drop (same probabilities you used before)
        int roll = new java.util.Random().nextInt(100) + 1;
        String dropName;
        if (roll < 50) {
            dropName = sample.demo2.object.OBJ_Coin_Bronze.objName;
        } else if (roll < 75) {
            dropName = sample.demo2.object.OBJ_Heart.objName;
        } else {
            dropName = sample.demo2.object.OBJ_ManaCrystal.objName;
        }

        // SERVER/HOST: authoritative spawn (broadcasts to everyone)
        if (gp.gameServer != null) {
            gp.gameServer.spawnObjectAt(dropName, worldX, worldY);
            return;
        }

        // GUEST: do nothing here; server will spawn/broadcast and snapshot
        if (gp.gameClient != null) {
            return;
        }

        // SINGLE-PLAYER: spawn locally
        sample.demo2.entity.Entity item = gp.eGenerator.getObject(dropName);
        if (item != null) {
            dropItem(item);
        }
    }
    ...
    public void checkAttackOrNot(int rate, int straight, int horizontal)
    {
        ...
        {
            case "up":
                if(gp.player.getCenterY() < getCenterY()  && yDis < straight && xDis < horizontal) { tartgetInRange = true; }
                break;
            case "down":
                if(gp.player.getCenterY()  > getCenterY()  && yDis < straight && xDis < horizontal) { tartgetInRange = true; }
                break;
            case "left":
                if(gp.player.getCenterX()  < getCenterX() && xDis < straight && yDis < horizontal) { tartgetInRange = true; }
                break;
            case "right":
                if(gp.player.getCenterX() > getCenterX() && xDis < straight && yDis < horizontal) { tartgetInRange = true; }
                break;
    ...
    public void draw(GraphicsContext gc)
    {
        ...
        if(inCamera() == true)
        {
            ...
            switch (direction)
            {
                case "up":
                    if(!attacking) {
                        ...
                        tempScreenY = getScreenY() - (int)up1.getHeight();
                ...
                case "left":
                    if(!attacking) {
                        ...
                        tempScreenX = getScreenX() - (int)left1.getWidth();
            ...
            gc.setGlobalAlpha(originalAlpha); // Reset alpha
        }
    }
    ...
    public Image setup(String imagePath, int width, int height) {
        try {
            String url = getClass().getResource(imagePath + ".png").toExternalForm();
            // backgroundLoading = true (last arg)
            return new Image(url, width, height, false, false, true);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
